...
public class PipeWorkModifyFragment extends BaseFragment {

    public static final String TAG = "PipeWorkModifyFragment";

    private static final String PIPEWORK_ID = "pipeWork";
    FragmentPipeWorkModifyBinding mBinding;
    private PipeWork curPipeWork;
    private PipeWorkViewModel pipeWorkViewModel;
    private CommonDialogFragment commonDialogFragment;
    private Disposable curDisposable;
    public static ObservableField<PipeWork> pipeWorkField;

    public static PipeWorkModifyFragment newInstance(PipeWork pipeInfo) {
        PipeWorkModifyFragment fragment = new PipeWorkModifyFragment();
        Bundle bundle = new Bundle();
        bundle.putSerializable(PIPEWORK_ID, pipeInfo);
    ...
    public void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        if (null != getArguments()) {
            curPipeWork = (PipeWork) getArguments().getSerializable(PIPEWORK_ID);
        }
    }
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle
            savedInstanceState) {
        mBinding = DataBindingUtil.inflate(inflater, R.layout.fragment_pipe_work_modify, container, false);
        setToolBarHeight(mBinding.toolbar.getRoot());
        mBinding.setFragment(this);
        ToolbarInfo toolbarInfo = new ToolbarInfo();
        toolbarInfo.setTitle(getString(R.string.title_pipe_work_modify));
    ...
    private void initView() {
        pipeWorkField = new ObservableField<>();
        pipeWorkField.set(curPipeWork);
        mBinding.setMyFragment(this);

        mBinding.modifyPw.setOnClickListener(view -> {
            closeDisposable();
                ...
                if (getLifecycle().getCurrentState().isAtLeast(Lifecycle.State.RESUMED)) {
                    if (null != aBoolean) {
                        if (aBoolean) {
                            ...
                            getFragmentManager().popBackStackImmediate(PipeWorkDetailFragment.TAG, FragmentManager
